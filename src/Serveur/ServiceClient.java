...
public class ServiceClient implements Runnable, Observer {
	...
	// Elements envoyés par le client vers le serveur poour initialiser des
	// attributs important
	...
	public void run() {
		 ...
		 * while(nomClient.equals("")){ try { email = flux_entrant.readLine(); joueur =
		 * user.getUtilisateur(email, "pute"); if(joueur == null){
		 * ma_sortie.println("[Serveur]: Mauvaise adresse mail : réessayer !"); } else
		...
		//TEST
		ArrayList<InfoItem> infosItems = new ArrayList<>();
		infosItems.add(new InfoItem(1,2,ItemType.BOMB_DOWN));

		InfoBomb ib=new InfoBomb(3, 4, 5, StateBomb.Step1, null);
		ArrayList<String> infosBomb = new ArrayList<>();
		infosBomb.add(ib.toText());
		sendObject.setInfoGame(map.get_walls(),infosItems,infosBomb);
	...
	public void update(Observable observable, Object o) {
		...
		//this.sendObject.setInfoGame(this.game.getEtatJeu().getBrokable_walls(), this.game.getEtatJeu().getBombermans(),
				//this.game.getEtatJeu().getItems(), this.game.getEtatJeu().getBombs());
		/*
		 * if(game.isEndgame()){ ma_sortie.println("[Serveur]: Fin de la partie : ");
		 * for(Agent b : this.game.getEtatJeu().getBombermans()) { Bomberman temp =
		 * (Bomberman) b; ma_sortie.println("[Serveur]: Score de "+temp.getColor()+" : "
		 * + temp.score); }
		 * 
		 * }
		 */
